...
@interface ZZNewsSheetMenu1(){
    CGPoint _lastPoint;
    ZZNewsSheetItem1 *_currentItem;
    ZZNewsSheetItem1  *_placeHolderItem;
}
...
@property(nonatomic,strong)NSMutableArray<ZZNewsSheetItem1*> *mySubjectItemArray;
@property(nonatomic,strong)NSMutableArray<ZZNewsSheetItem1*> *recommendSubjectItemArray;
...
@implementation ZZNewsSheetMenu1
...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        _placeHolderItem = [ZZNewsSheetItem1 new];
        [self commit];
    }
    return self;
}
...
//    lab.text = @"导航设置";
    ...
    for (int i = 0; i<self.mySubjectArray.count; i++) {
        ZZNewsSheetItem1 *item = [[ZZNewsSheetItem1 alloc]init];
        ...
        [item setItemCloseBlock:^(ZZNewsSheetItem1 *item) {
...
- (void)layoutSubviews{
    ...
    lab.textColor = RGB(51, 51, 51);
    lab.font = [UIFont boldSystemFontOfSize:15];
    CGSize size = [ZZNewsSheetConfig1 defaultCofing].sheetItemSize;
    NSInteger column = [ZZNewsSheetConfig1 defaultCofing].sheetMaxColumn;
...
- (void)updateMoveItem:(ZZNewsSheetItem1 *)item{
    ...
    if ([ZZNewsSheetConfig1 defaultCofing].isHiddenWhenHasNoneRecomment) {
...
- (void)moveItemFromRecommendToMySubject:(ZZNewsSheetItem1*)item{
...
- (void)setUpGrimViews{
    ...
    [self.mySubjectItemArray enumerateObjectsUsingBlock:^(ZZNewsSheetItem1 *  _Nonnull obj, NSUInteger i, BOOL * _Nonnull stop) {
...
- (void)updateAllView{
    ...
    for (NSInteger i = 0;i<self.mySubjectItemArray.count;i++) {
        ZZNewsSheetItem1 *item = self.mySubjectItemArray[i];
        NSInteger currentRow = i / column ;
        NSInteger currentColumn = i % column;
        CGFloat orx = margin + currentColumn *margin + currentColumn*itemWidth;
//        CGFloat ory = titleHeight + margin + currentRow*margin + currentRow*itemHeight;
    ...
    for (NSInteger i = 0;i<self.recommendSubjectItemArray.count;i++) {
        ZZNewsSheetItem1 *item = self.recommendSubjectItemArray[i];
        NSInteger currentRow = i / column ;
        NSInteger currentColumn = i % column;
        CGFloat orx = margin + currentColumn *margin + currentColumn*itemWidth;
...
- (void)hiddenAllFlag:(BOOL)hidden{
    for (UIView * mv in self.mySubjectView.subviews) {
        if ([mv isKindOfClass:[ZZNewsSheetItem1 class]]) {
            ZZNewsSheetItem1 * item = (ZZNewsSheetItem1 *)mv;
...
- (void)itemLongPress:(UILongPressGestureRecognizer *)gesture{
    ZZNewsSheetItem1 *item = (ZZNewsSheetItem1 *)gesture.view;
    ...
    [self.mySubjectItemArray enumerateObjectsUsingBlock:^(ZZNewsSheetItem1 *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
-(void)updateNewSheetConfig:(newsSheetBlock)block{
    ZZNewsSheetConfig1 *cofig = [ZZNewsSheetConfig1 defaultCofing];
    if (block) {
         block(cofig);
    }
