...
@interface DFSegmentView1 ()<DFSegmentHeadViewDelegate,UIScrollViewDelegate>

@property (nonatomic, strong) DFSegmentHeadView1 *dfSegmentHead;
...
- (void)prepareLayout {
    ...
    self.dfSegmentHead = [[DFSegmentHeadView1 alloc] initWithFrame:CGRectZero];
    ...
    [self.dfSegmentHead mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.mas_equalTo(kScreen_Width-44);
    }];
...
- (NSString *)textForCellWithIndex:(NSInteger)index {
    ...
    if ([[self.titleArr objectAtIndex:index] isKindOfClass:[NSNull class]]) {
        return @"";
    }else{
